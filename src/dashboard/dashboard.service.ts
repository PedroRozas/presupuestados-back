...
import { MonthlyFinanceService } from '../monthly-finance/monthly-finance.service.js';

@Injectable()
export class DashboardService {
  constructor(
    @Inject(DRIZZLE) private readonly db: NodePgDatabase<typeof schema>,
    private readonly monthlyFinance: MonthlyFinanceService,
  ) {}
  ...
  async getDashboardData(coupleId: string, month: number, year: number) {
    const [members, incomesRes, deductionsRes, budgetsList, monthExpenses] =
      await Promise.all([
        this.getMembers(coupleId),
        this.monthlyFinance.getEffectiveIncomes(coupleId, month, year),
        this.monthlyFinance.getEffectiveDeductions(coupleId, month, year),
    ...
    return {
      members,
      incomes: incomesRes.effective.map((e) => ({
        id: e.id,
        userId: e.userId,
        amount: e.amount,
      ...
      deductions: deductionsRes.effective.map((e) => ({
        id: e.id,
        userId: e.userId,
        amount: e.amount,
        description: e.description,
      ...
      incomesPersonalized: incomesRes.personalized,
      deductionsPersonalized: deductionsRes.personalized,
    };
  }
  ...
  async getSummary(coupleId: string, month: number, year: number) {
    const [members, incomesRes, deductionsRes, monthExpenses] =
      await Promise.all([
        this.getMembers(coupleId),
        this.monthlyFinance.getEffectiveIncomes(coupleId, month, year),
        this.monthlyFinance.getEffectiveDeductions(coupleId, month, year),
    ...
    const allIncomes = incomesRes.effective;
    const allDeductions = deductionsRes.effective;
